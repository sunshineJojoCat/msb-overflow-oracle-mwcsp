...
% TODO(pre-submission): Add second author's (Kamonluk Suksen) email to the
% address line below, or confirm with the venue that only the corresponding
% author requires an email.
\address[1]{Department of Computer Engineering, Chulalongkorn University, Bangkok, Thailand 10330 (email: witchapat.iss@student.chula.ac.th)}

\corresp{Corresponding author: Witchapat Issariyawat (email: witchapat.iss@student.chula.ac.th).}

\begin{abstract}
Solving the Maximum Weight Colorable Subgraph Problem (MWCSP) on near-term quantum hardware is constrained by qubit count and circuit depth. Grover's Algorithm offers a quadratic speedup for combinatorial search, but traditional exact-match oracles rely on deep Multi-Controlled X (MCX) gates to verify cost constraints of the form $\text{score} = \text{target}$. We propose a space-efficient architecture based on an Overflow-Flag Range Comparator. Appending one Most Significant Bit (MSB) qubit to the weight accumulator and initializing the register with a classically computed offset ($\text{Offset} = 2^{S-1} - \text{Target}$) causes any configuration meeting the target score to overflow the accumulator and flip the MSB. The phase-kickback layer therefore reduces from an $\mathcal{O}(S^2)$ MCX network to a single CNOT on the MSB. Combined with a Custom State Preparation procedure that prunes invalid color permutations from the initial superposition, our Overflow-Flag Oracles save qubits relative to per-edge flag-ancilla baselines and collapse the $\mathcal{O}(S^2)$ MCX comparator network to a single CNOT, at the cost of a per-edge ripple-carry adder specific to the weighted formulation. Empirical benchmarking across five graph topologies confirms that the design provides a practical, space-efficient route for exact range-based combinatorial search on early fault-tolerant, space-constrained quantum processors.
\end{abstract}
...
%
\begin{enumerate}
    \item A Custom State Preparation operator that initializes the
    Grover search over $K^N$ valid colorings, requiring
    $\mathcal{O}(\log K)$ gates per node and no ancilla qubits
    (Section~\ref{sec:methodology}).

    \item An MSB-Overflow Range Comparator that detects score
    $\geq T$ with a single CNOT, replacing the $\mathcal{O}(S^2)$
    MCX comparator of exact-match designs
    (Section~\ref{sec:methodology}).

    \item Three oracle variants---Min Width, Min Depth, and
    Balanced---with the optimal ancilla count $C^*$ derived
    analytically to minimize the space-time cost product
    (Section~\ref{sec:methodology}).

    \item Empirical validation via Qiskit AerSimulator transpilation
    across five weighted graph topologies, with correctness confirmed
    against classical brute-force enumeration
...
To map this complex combinatorial structure into a universal quantum environment natively suitable for discrete computational basis states, the color of each individual node must be mapped explicitly to an isolated local register of computational hardware qubits. Based on standard binary logarithmic scaling \cite{saha2020}, each isolated node requires exactly $q = \lceil \log_2(K) \rceil$ physical qubits solely to encompass the necessary range of $K$ available colors.

Let $|c\rangle$ represent the tensor state vector of a single node's color registry, scaled such that $|c\rangle = |q_{n-1} \dots q_0\rangle \in \{0, 1\}^q$. If the operational algorithm strictly relies on standard generalized computational mapping, the entire system initializes by enforcing a global parallel Hadamard transformation ($H^{\otimes (N \cdot q)}$) across all initialization registers. This mathematically forces a perfectly uniform binary superposition spanning exactly $2^{N \times q}$ absolute tensor states uniformly distributed with probability amplitudes of $1/\sqrt{2^{Nq}}$.

A spatial mismatch emerges whenever $K$ is not a power of $2$. For example, $K=3$ requires $q=2$ qubits per node; under standard Hadamard initialisation the phantom state $|11\rangle$---which would correspond to colour value $3$, outside the valid range $\{0,1,2\}$---appears in the global superposition with equal amplitude to the valid colours, despite having no classical counterpart.

This oversight structurally causes severe Amplitude Leakage. If $N$ nodes expand outward via generic Hadamards, the total size of the pure mathematical search topology $M_{\text{total}} = 2^{Nq}$ vastly dwarfs the size of the actually valid target space $M_{\text{valid}} = K^N$. Consequently, the Grover phase-kickback mechanism wastes countless iteration cycles endlessly amplifying strictly impossible configurations. Our theoretical formulation explicitly remedies this significant resource sink directly at the source by establishing an exclusive, tightly mathematically bounded geometric initial superposition space long prior to executing any of the numerical accumulation depth logic.
...
Before applying the arithmetic quantum weight additions, the circuit must initialize the theoretically valid states. As established in prior unweighted baseline optimizations \cite{belletti2025}, applying standard localized Hadamard gates ($H^{\otimes q_n}$) generates a generic full binary superposition containing mathematically impossible color permutations. We adopt the uniform superposition construction of Shukla and Vedula~\cite{shukla2024} as the state preparation layer of our oracle. Their method produces a uniform superposition over $|0\rangle$ to $|K-1\rangle$ via $R_y$ rotations and controlled-Hadamard gates, with zero amplitude on invalid states. Our contribution is the integration of this operator as $\mathcal{A}_\mathrm{init}$ within the Grover oracle, replacing the Hadamard initialization of prior designs and eliminating the need for invalid-color checker subcircuits. Consequently, this projective formulation actively nullifies and zeroes out any mathematical probability amplitudes intrinsically mapping to invalid residual modulo states (such as $|11\dots\rangle$). This precise sequence explicitly loads the search space strictly targeting bounded valid $K^N$ permutations. 

Mathematically, our initialization operator $\mathcal{A}_{\text{init}}$ serves as a geometric rotational mapping acting strictly on the $q$-qubit subspace of each node:
\begin{equation}
\mathcal{A}_{\text{init}} |0\rangle^{\otimes q} = \frac{1}{\sqrt{K}} \sum_{c=0}^{K-1} |c\rangle
\end{equation}
By executing this operation uniformly across all $N$ vertices, the global system initializes cleanly into:
\begin{equation}
...
\end{equation}
where $\mathcal{V}$ defines the strict set of configurations harboring no local node color exceeding $K-1$. This algorithm intuitively handles geometric edge cases where $K$ is not a power of 2, physically dropping the architectural requirement for multi-qubit invalid-color evaluation constraints mid-algorithm, freeing critical space for the weight accumulators.
...
\paragraph{Derivation of $C^*$} The optimal ancilla count $C^*$ arises from minimizing the total space-time cost function $\Phi(C) = (N \cdot q + S + C) \times \lceil |E|/C \rceil$, representing the product of qubit width and circuit depth. Treating the ceiling as a continuous approximation $|E|/C$ and differentiating, $\partial \Phi / \partial C = 0$ yields the quadratic $C^2 + C - |E| = 0$, whose positive root is $(-1 + \sqrt{1 + 4|E|})/2$. Taking the ceiling for an integer ancilla count:
\begin{equation}
C^* = \left\lceil \frac{-1 + \sqrt{1 + 4|E|}}{2} \right\rceil
\label{eq:cstar}
\end{equation}
Intuitively, $C^*$ balances two competing costs: adding ancillae reduces depth (fewer batches) but increases width. The square-root scaling means that doubling the edge count only increases the optimal ancilla count by approximately $\sqrt{2}$, ensuring the Balanced variant scales gracefully.
...
Across all five topologies and all three oracle variants (Min Width, Min Depth, Balanced), the quantum algorithm consistently identified the exact global maximum with dominant measurement probability. Crucially, this correctness extends to the Frustrated $K_4$ graph---a topology where conventional Belletti-style oracles would return exactly zero valid solutions, as no proper 3-coloring exists for $K_4$. Our MSB-Overflow architecture correctly identifies the configuration maximizing the sum of properly colored edges, demonstrating the inherent superiority of range-based ($\ge$) evaluation over exact-match ($==$) approaches.
...
{Grover measurement outcome distribution for the Frustrated $K_4$ graph ($K\!=\!3$, 2 iterations) with edge weights $\{(0,1,4),(0,2,5),(0,3,3),(1,2,6),(1,3,2),(2,3,4)\}$. The x-axis represents the raw quantum bitstring measurements, which map directly to the corresponding graph coloring configurations. Green bars indicate target optimal states (score $= 22$, six valid colourings), amplified to dominant probability. The dashed red line marks the uniform baseline $1/K^N = 1/81$.\label{fig:measurement}}
...
QAOA formally possesses no mathematical guarantee of converging onto the global maximum. Its solutions are bounded by an approximation ratio dependent on the layered circuit depth $p$. In contrast, our Overflow-Flag Grover Oracle guarantees complete amplitude amplification towards the global maximum, with the optimal iteration count bounded at $\lfloor \frac{\pi}{4}\sqrt{M / |\mathcal{S}^*|} \rfloor$, where $|\mathcal{S}^*|$ is the number of optimal solutions.
...
This paper addresses a key structural bottleneck limiting the use of exact Grover Oracles for the Maximum Weight Colorable Subgraph Problem (MWCSP) on space-constrained superconducting hardware. By replacing the deep MCX-based exact-match comparator with an MSB Overflow-Flag range comparator, we collapse the phase-kickback validation depth.

Initializing the accumulator with the classical offset $\text{Offset} = 2^{S-1} - \text{Target}$ reduces the $\mathcal{O}(S^2)$ comparator to a single CNOT executing in $\mathcal{O}(1)$ depth. Combined with space-efficient ripple-carry weight accumulators and a custom state preparation that emits only valid color configurations, this design demonstrates a concrete architectural path for executing exact integer Grover searches at scale, eliminating the dominant MCX overhead.
...
All source code used to generate the results, tables, and figures in this paper is openly available. The package includes the three oracle constructors (Min Width, Min Depth, Balanced) and the classical brute-force verifier (\texttt{verify\_table2.py}); the empirical scaling harness for Table~\ref{tab:scaling_measured} (\texttt{extend\_table\_scaling.py}); the depolarising-noise sweep that produced Table~\ref{tab:noisy} (\texttt{noisy\_simulation.py}); the QAOA $p{=}7$ baseline that produced the QAOA columns of Table~\ref{tab:empirical_scaling} (\texttt{qaoa\_depth\_sweep.py}); and the Belletti-style Boolean oracle reimplementation used in Table~\ref{tab:belletti_compare} (\texttt{belletti\_baseline.py}). The \LaTeX{} source, the BibTeX bibliography, and all eight published PNG figures are also bundled so that the PDF can be rebuilt from scratch.

\begin{itemize}
  \item \textbf{Source repository:} \url{https://github.com/sunshineJojoCat/msb-overflow-oracle-mwcsp}
  \item \textbf{Archived snapshot (this version):} Zenodo, \texttt{DOI:10.5281/zenodo.XXXXXXX}
\end{itemize}

%
% TODO(pre-submission): Replace the Zenodo DOI placeholder with the actual
% DOI minted from the v1.0-tqe-submission release tag.
%
